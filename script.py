...
while True:
    for locationId, location in locations.items():
        query = """curl -s 'https://api.myturn.ca.gov/public/locations/""" + locationId + """/availability' \
        ...
        current_time = datetime.datetime.now().astimezone(timezone('US/Pacific')).strftime(date_format)
        print(Back.WHITE + Fore.BLACK + current_time + Style.RESET_ALL)
        if 'false' not in result:
            print (Back.LIGHTRED_EX + Fore.BLUE + "Update Vaccine Data:" + Style.RESET_ALL)
            beepy.beep(sound='robot_error')
            vaccineData = input()
        elif 'true' in result:
            print (Back.LIGHTGREEN_EX + Fore.RED + "Vaccine Available at " + location + Style.RESET_ALL)
            beepy.beep(sound='success')
        else:
            print (Back.LIGHTYELLOW_EX + Fore.MAGENTA + "Vaccine Not Available at " + location + Style.RESET_ALL)
        time.sleep(5)
